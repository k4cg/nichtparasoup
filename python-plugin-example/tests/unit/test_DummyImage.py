...
class TestDummyImageConfigWidth:
    ...
    @pytest.mark.parametrize('wrong_type', [None, True, '', 0.001, list(), set(), dict(), tuple(), object()], ids=type)
    ...
    @pytest.mark.parametrize('wrong_value', [0, -23], ids=['zero', 'negative'])
...
class TestDummyImageConfigHeight:
    ...
    @pytest.mark.parametrize('wrong_value', [0, -42], ids=['zero', 'negative'])
    def test__check_config_wrong_value(self, wrong_value: int) -> None:
        # arrange
        config_in = _DUMMYIMAGE_RIGHT_CONFIG.copy()
