...
import pytest
from _pytest.tmpdir import TempdirFactory  # type: ignore
...
class TestRemoteFetcher:
    ...
    def test_get_stream(self, use_debug_store_dir: bool, tmpdir_factory: TempdirFactory) -> None:
        ...
        # arrange
        tmp_dir = tmpdir_factory.mktemp(__name__).strpath if use_debug_store_dir else None
        if tmp_dir:
            assert len(os.listdir(tmp_dir)) == 0
        rf = RemoteFetcher()
        rf._debug_store_dir = tmp_dir
    ...
    def test__debug_get_store_dir(self, tmpdir_factory: TempdirFactory) -> None:
        ...
        not_exist = tmp_dir.join('not_exist').strpath
        a_file = tmp_dir.join('a_file').strpath
        open(a_file, 'x').close()
        # act
        res_no_dir = RemoteFetcher._debug_get_store_dir(None)
        res_a_file = RemoteFetcher._debug_get_store_dir(a_file)
        res_not_exist = RemoteFetcher._debug_get_store_dir(not_exist)
        res_a_dir = RemoteFetcher._debug_get_store_dir(tmp_dir)
        # assert
        assert res_no_dir is None
        assert res_a_file is None
        assert res_not_exist is None
        assert res_a_dir is not None
        assert res_a_dir == os.path.abspath(res_a_dir)
